...
export function validateCommandResult(result, cmdObj) {
  ...
  if (result.code !== 0) {
    hasError = true
    logger.error(`🔴 命令 "${cmd}" 退出码非 0：${result.code}`, logCache)
  }

  // 2. 判断是否要求 stderr 视为错误
  if (exitOnStdErr && result.stderr) {
    hasError = true
    logger.error(`🔴 命令 "${cmd}" 输出错误信息（stderr）被视为失败`, logCache)
  }
  ...
  if (
    ...
    hasError = true
    logger.error(`🔴 命令 "${cmd}" 匹配到错误模式：${errorMatch}`, logCache)
  }

  // 统一输出失败信息
  if (hasError) {
    logger.error(`❌ 命令失败：${description || cmd}`, logCache)
  }
